...
public class ConfigManager {
    ...
    private static void saveConfig() {
        try {
            ...
            // 宿主内建主题扩展键（theme.*）：随宿主合并写回一并持久化（保留未知键策略不变）
            for (Object keyObj : properties.stringPropertyNames()) {
    ...
    /** 读主题键（不含前缀 theme.），缺省返回 def。 */
    public static String getThemeProperty(String key, String def) {
        String v = properties.getProperty("theme." + key);
        return v != null ? v : def;
    }

    /** 写主题键（不含前缀 theme.）；值为 null 表示移除该键。写后整体落盘。 */
    public static void setThemeProperty(String key, String value) {
        if (key == null || key.isEmpty()) {
            return;
        }
        if (value == null) {
            properties.remove("theme." + key);
        } else {
            properties.setProperty("theme." + key, value);
        }
        saveConfig();
    }
}
