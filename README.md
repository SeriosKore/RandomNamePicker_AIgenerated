...
- 最新版本 **v1.1.1** 修复与增强：子球取消尾迹、插件未安装时隐藏相关设置、画笔附着悬浮球并支持鼠标绘制、插件管理界面（详见 `CHANGELOG.md` 与发布说明）；
- 上一个版本 **v1.1.0** 新增：名单修改日志、开机自启增强、多人点名、插件生态；
...
- **悬浮球多球动画由“多悬浮球插件”提供**（见 2.13）：中奖者小球围绕主球弹出/旋转/浮动、多环防重叠、透明度可调，**子球干净渲染无尾迹**；
- **设置联动**：未安装或已禁用“多悬浮球插件”时，“单次抽取数量”设置项自动隐藏并置灰提示“未安装或未启用多球抽取插件”，抽取自动按单人处理。

### 2.13 插件生态（不修改主程序即可扩展功能）

- 将插件 JAR（清单文件含 `Plugin-Class` 属性）放入程序运行目录下的 `extensions/` 文件夹，程序启动时**自动扫描并加载**；
- **插件管理界面**：主窗口“插件”菜单与悬浮球右键菜单均有“插件管理”入口，可查看已安装插件列表（名称/版本/状态：启用中/已禁用/未安装）、**启用/禁用**（即时生效，设置界面选项自动联动显示/隐藏）、**卸载**（删除 JAR）、**安装**（选择 JAR 复制到 extensions 并立即加载）；
...
- 随仓库发布的插件：
  ...
  - **屏幕画笔插件** `PenPlugin`：参考 Seewo 白板悬浮球画笔——**画笔入口附着在悬浮球上**（铅笔图标随球移动、点击展开/收起工具条，不影响悬浮球拖拽与点名），**鼠标/触笔/触摸均可书写**，全屏透明画布置顶绘制，笔锋随速度/压感变化、渐细起收笔，触屏大面积接触自动识别为橡皮（鼠标右键亦为橡皮）；
...
| `PluginManager` | 插件层 | 扫描 extensions/*.jar 并加载插件；维护各扩展点注册表；启用/禁用/卸载/安装；插件信息查询 |
| `PluginManagerDialog` | 表现层 | 插件管理界面：插件列表（名称/版本/状态）与启用、禁用、卸载、安装操作 |
...
│   ├── PluginManager.java        # 插件扫描/加载/注册/卸载/启用禁用/安装
│   ├── PluginContextImpl.java    # 插件上下文实现
│   ├── PluginManagerDialog.java  # 插件管理界面（查看/启用禁用/卸载/安装）
...
7. **多人动画**：中奖者小球姓名超过 6 字时以省略号展示（与单人模式截断逻辑一致），动画窗口点击或 15 秒后自动关闭（该动画现由“多悬浮球插件”提供，子球无尾迹）；
8. **插件权限**：插件与主程序运行在同一 JVM、共享全部权限，请仅加载可信来源的插件；
9. **插件管理**：启用/禁用即时生效，但系统托盘菜单项需重启程序后刷新；
10. **画笔插件**：画笔模式为全屏覆盖层，绘画期间会拦截全屏输入（悬浮球与其附着工具条保持在上层，可正常拖拽）；按 Esc 或工具条“停止画笔”结束；多屏仅覆盖主显示器；系统 DPI 缩放非 100% 时笔迹坐标可能有轻微偏移。
