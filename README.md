...
| `MultiPickBallWindow` | 表现层 | 多人点名动画窗口：主球体 + 中奖者小球弹出/旋转/浮动 |
| `ConfigWindow` | 表现层 | 名单管理对话框：表格增删、导入导出、保存 |
| `SettingsWindow` | 表现层 | 系统设置对话框：自启动、托盘策略、悬浮球外观、单次抽取数量、日志导出/查看、锁定/改密 |
...
> `data/`、`log/` 与 `modification_log.txt` 由程序首次运行时自动创建。
